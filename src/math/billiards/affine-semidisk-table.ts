...
export class AffineSemidiskTable extends AffineOuterBilliardTable {
  ...
  constructor(readonly beta: number) {
    ...
    this.slicingRays.push(
      new Straight(new Vector2(-this.x, this.y), new Vector2(-this.x - 1, this.y), true)
    );
  }
  ...
  time(point: Vector2): number {
    ...
    } else if (point.y > this.y - EPSILON && closeEnough(point.lengthSq(), 1)) {
      const theta = normalizeAngle(point.angle() - (Math.PI / 2 - this.beta));
      return theta / (Math.PI * 2 - 2 * this.beta) * this.curveTime;
    } else {
  ...
  // override preimages(flavor: Generator, iterations: number): Straight[] {
  //   switch (flavor) {
  ...
  // private sliceRegularPreimage(preimage: Straight): Straight[] {
  ...
  //     pieces.push(new Straight(p1, p2, false));
  //   }
  //   return pieces;
  // }
  //
  ...
  // private regularPreimages(iterations: number): Straight[] {
  ...
  // private symplecticPreimages(iterations: number): Straight[] {
  //   const preimages: Straight[] = [];
  //   let frontier: Straight[] = [];
  ...
  supportPoint(theta: number): Vector2 {
    ...
    if (theta >= 0 && theta <= this.beta) return this.rightPoint.clone();
    if (theta <= 0 && theta >= -this.beta) return this.leftPoint.clone();
    return new Vector2(
      Math.sin(theta),
      -Math.cos(theta),
    );
  ...
  override width(theta: number): number {
    ...
    const diff = sp2.sub(sp1);
    return Math.abs(diff.dot(new Vector2(
      -Math.sin(theta),
      Math.cos(theta),
    )));
  }
  ...
  boundary(point: Vector2): boolean {
    return this.pointOnBoundary(point);
  }
  ...
  tangentVector(time: number): Vector2 {
    return polar(1, this.tangentHeading(time) || this.tangentHeading(time + EPSILON) || 0);
  }
}