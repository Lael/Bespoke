import {SpherePoint, SphericalArc, SphericalCircle, sphericalLerp, SphericalPolygon} from "../geometry/spherical";
...
export class SphericalPolygonTable extends SphericalOuterBilliardTable {
  ...
  override intersect(ray: SphericalRay): number {
    return this.time(this.polygon.intersect(ray));
  }
  ...
  tangentVector(time: number): Vector3 {
    let l = fixTime(time) * this.polygon.perimeter;
    let t = 0;
    for (let i = 0; i < this.n; i++) {
      if (t === l) throw Error('hit a vertex');
      let a = this.polygon.arcs[i];
      let al = a.length
      if (t + al > l) {
        let alpha = (l - t) / al;
  ...
  override preimages(flavor: Generator, iterations: number): SphericalArc[] {
    ...
    const preimages = [];
    for (let a of this.polygon.arcs) {
      preimages.push(new SphericalArc(a.p1, a.p2));
  ...
  intersectSmallCircle(circle: SphericalCircle): SpherePoint[] {
    return [];
  }
}