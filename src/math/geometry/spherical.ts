...
import {SphericalRay} from "../billiards/spherical-billiard-table";
...
export class SphericalPolygon {
  ...
  boundary(point: SpherePoint) {
    for (let a of this.arcs) {
      if (a.containsPoint(point)) return true;
    }
    return false;
  }
  ...
  intersect(ray: SphericalRay): SpherePoint {
    if (!this.contains(ray.src)) throw Error('polygon does not contain ray source');
    const axis = ray.src.coords.clone().cross(ray.dir).normalize();
    const rayArc = new SphericalArc(ray.src, new SpherePoint(ray.src.coords.clone().applyAxisAngle(axis, 0.99 * Math.PI)));
    let best: SpherePoint | null = null;
    let bestDist = Number.POSITIVE_INFINITY;
    for (let a of this.arcs) {
      const p = a.intersectArc(rayArc);
      if (!p) continue;
      const d = p.distanceTo(ray.src)
      if (d < bestDist) {
        bestDist = d;
        best = p;
      }
...
export class SphericalCircle {
  ...
  static withTangent(p: SpherePoint, v: Vector3, q: SpherePoint): SphericalCircle {
    const normal = v.clone().cross(q.coords.clone().sub(p.coords)).normalize();
    const d = normal.dot(p.coords);
    return new SphericalCircle(new SpherePoint(normal), Math.acos(d));
  }

  static throughThreePoints(p1: SpherePoint, p2: SpherePoint, p3: SpherePoint): SphericalCircle {
    const v1 = p2.coords.clone().sub(p1.coords);
    const v2 = p3.coords.clone().sub(p1.coords);
    const normal = v1.cross(v2).normalize();
    const d = normal.dot(p1.coords);
    return new SphericalCircle(new SpherePoint(normal), Math.acos(d));
  }
}
