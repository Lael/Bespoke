...
import {affinity as affinityHelper, closeEnough} from "../../../math/math-helpers";
...
@Component({
  ...
  styleUrls: ['../../widgets/three-demo/three-demo.component.sass', 'symplectic-table.component.sass'],
  standalone: true,
  imports: [CommonModule]
})
export class SymplecticTableComponent extends PolygonPickerComponent implements AfterViewInit, OnDestroy {
  // coordinate view
  affineScene: Scene;
  affineRenderer: WebGLRenderer;
  affineCamera: OrthographicCamera;

  @ViewChild('affine_container', {static: true})
  affineHostElement?: ElementRef;
  affineOrbitControls: OrbitControls;

  n = 5;
  iterations = 10;
  everyOther: boolean = false;
  rescale: boolean = false;
  convex: boolean = true;
  inner: boolean = true;
  drawVertices: boolean = true;
  ...
  constructor() {
    ...
    this.affineScene = new THREE.Scene();
    this.affineRenderer = new THREE.WebGLRenderer({
      antialias: true,
    });
    this.affineRenderer.setPixelRatio(window.devicePixelRatio);
    this.affineRenderer.setClearColor(this.getColor('clear'));

    this.affineCamera = new OrthographicCamera();
    this.affineCamera.position.z = 100;

    this.affineOrbitControls = new OrbitControls(this.affineCamera, this.affineRenderer.domElement);
    this.affineOrbitControls.enableRotate = false;
    this.affineOrbitControls.zoomToCursor = true;
  ...
  updateGUI() {
    ...
    this.gui.add(this, 'drawVertices').name('Vertices').onFinishChange(() => {
      this.markDirty();
    });
  ...
  override ngAfterViewInit() {
    if (!this.affineHostElement) {
      console.error('Missing container for coordinate renderer');
      return;
    }
    this.affineRenderer.setSize(400, 400);
    this.affineOrbitControls = new OrbitControls(this.affineCamera, this.affineRenderer.domElement);
    this.affineOrbitControls.enableRotate = false;
    this.affineOrbitControls.zoomToCursor = true;
    this.affineHostElement.nativeElement.appendChild(this.affineRenderer.domElement);
  ...
  override frame(dt: number) {
    ...
    if (this.showAffine) {
      this.affineScene.clear();
      this.affineScene.add(this.affineFinal);
      this.affineRenderer.setClearColor(this.getColor('clear'));
      this.affineRenderer.render(this.affineScene, this.affineCamera);
    }
  }
  ...
  iterate() {
    ...
    if (this.inner) {
      const at = mapThree(finalPolygon);
      ...
      const apc = new Vector2();
      for (let a of affinePoints) {
        apc.add(a);
      }
      apc.multiplyScalar(1.0 / affinePoints.length);
...
function mapThree(vertices: Vector2[]): Matrix3 {
  const n = vertices.length;
  if (n === 6) {
    return affinityHelper([vertices[0], vertices[2], vertices[4]], [
      Complex.polar(0.5, Math.PI / 2).toVector2(),
      Complex.polar(0.5, 7 * Math.PI / 6).toVector2(),
      Complex.polar(0.5, 11 * Math.PI / 6).toVector2(),
    ]);
  } else if (n >= 3) {
    return affinityHelper([vertices[0], vertices[1], vertices[2]], [
      Complex.polar(1, Math.PI - 2 * Math.PI / n).toVector2(),
      new Vector2(),
      new Vector2(1, 0),
    ]);
  } else {
