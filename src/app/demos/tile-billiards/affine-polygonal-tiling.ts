...
import {EuclideanPolygon, EuclideanPolygonRayCollision} from "../../../math/geometry/euclidean-polygon";
import {EuclideanRay} from "../../../math/geometry/euclidean-ray";
import {Complex} from "../../../math/complex/complex";
import {EPSILON} from "../../../math/math-helpers";

export abstract class AffinePolygonalTiling extends PolygonalTiling<AffineTile, EuclideanPolygon> {
  ...
  override play(iterations: number, start: Vector2, direction: number) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      let collision: EuclideanPolygonRayCollision;
      try {
        collision = p.castRay(ray);
      } catch (e) {
        console.log(`broke at iteration ${i}:`, e);
        break;
      }
  ...
  private polygonForTile(t: AffineTile): EuclideanPolygon {
